...
SELECT  LUONG
FROM GIAOVIEN GV 
WHERE GV.MABM='HTTT' AND LUONG >= ALL ( SELECT LUONG FROM GIAOVIEN)
...
WHERE GV.MAGV=BM.TRUONGBM AND BM.TRUONGBM=DT.MAGV
GROUP BY GV.MAGV,GV.HOTEN 
HAVING COUNT (DT.MAGV) >=1
...
GROUP BY TENKHOA
HAVING COUNT (MAGV) >= ALL ( SELECT COUNT (*)
						  FROM GIAOVIEN GV
						  GROUP BY GV.MABM)
...
HAVING COUNT (  TG.MADT) >= ALL ( SELECT COUNT ( TG.MADT)
										 FROM  THAMGIADT TG
										 GROUP BY TG.MADT,MAGV)
--Q57. Cho biết tên trưởng bộ môn mà chủ nhiệm nhiều đề tài nhất.
SELECT GV.HOTEN 
FROM GIAOVIEN GV , BOMON BM ,DETAI DT 
WHERE GV.MAGV=BM.TRUONGBM AND BM.TRUONGBM=DT.GVCNDT
GROUP BY GV.MAGV,GV.HOTEN 
HAVING COUNT (GV.MAGV) >=ALL (SELECT COUNT(GVCNDT) 
							  FROM DETAI
							  GROUP BY GVCNDT)
